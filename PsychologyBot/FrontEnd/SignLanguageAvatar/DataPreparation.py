...
def get_hand_landmarks(frame, prev_frame=None):
    try:
        rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
    except cv2.error:
        if prev_frame is not None:
            return get_hand_landmarks(prev_frame, prev_frame)
        else:
...
def ExtractAlphabeticalDataset(video_folder, resize_dim=(224, 224), augment_count=3):
    ...
    for video in videos:
        ...
        for frame in frames:
            frame_data=transform(Image.fromarray(frame.astype(np.uint8)))
            landmarkdata=get_hand_landmarks(frame_data)
            transformed_frames.append(landmarkdata)
        ...
        for i in range(1,augment_count):
            ...
            for aug in augmented_data:
                frame_dataug=transform(Image.fromarray(aug.astype(np.uint8)))
                landmarkdataaug=get_hand_landmarks(frame_dataug)
                aug_transformed.append(landmarkdataaug)
